...
class Model:
    def __init__(self, local_rank,device,model_path):
        ...
        self.net.to('cuda:{}'.format(self.device_id))
    ...
    def load_model(self, name=None, rank=0):
        ...
        if rank <= 0 :
            if name is None:
                name = self.name
            self.net.load_state_dict(convert(torch.load(self.model_path)))
    
    def save_model(self, rank=0):
        if rank == 0:
            torch.save(self.net.state_dict(),f'./EMA/ckpt/{self.name}.pkl')
