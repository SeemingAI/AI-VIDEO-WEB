...
from .typings import Frame, Mask, Padding, FaceMaskRegion, ModelSet

from Obtain_Path import resolve_relative_path
import moudel_fuction.moudel_reference_fuction.facefusion.model as cfg
...
MODELS : ModelSet =\
{
	'face_occluder':
	{
		...
		'path': resolve_relative_path('moudel_fuction/moudel_reference_fuction/facefusion/model/ckpt/face_masker/face_occluder.onnx')
	},
	'face_parser':
	{
		'url': 'https://github.com/facefusion/facefusion-assets/releases/download/models/face_parser.onnx',
		'path': resolve_relative_path('moudel_fuction/moudel_reference_fuction/facefusion/model/ckpt/face_masker/face_parser.onnx')
	}
}
...
def get_face_occluder() -> Any:
	...
	with THREAD_LOCK:
		if FACE_OCCLUDER is None:
			...
			FACE_OCCLUDER = onnxruntime.InferenceSession(model_path, providers = apply_execution_provider_options(cfg.execution_providers))
...
def get_face_parser() -> Any:
	...
	with THREAD_LOCK:
		if FACE_PARSER is None:
			...
			FACE_PARSER = onnxruntime.InferenceSession(model_path, providers = apply_execution_provider_options(cfg.execution_providers))
	return FACE_PARSER
