...
from .face_helper import warp_face_by_kps, create_static_anchors, distance_to_kps, distance_to_bbox, apply_nms
from .execution_helper import apply_execution_provider_options
from .typings import Frame, Face, FaceSet, FaceAnalyserOrder, FaceAnalyserAge, FaceAnalyserGender, ModelSet, Bbox, Kps, Score, Embedding
from moudel_fuction.moudel_reference_fuction.facefusion.model import globals as cfg

from core.utils.vision import resize_frame_resolution, unpack_resolution
...
MODELS : ModelSet =\
{
	'face_detector_retinaface':
	{
		...
		'path': resolve_relative_path('moudel_fuction/moudel_reference_fuction/facefusion/model/ckpt/face_analyser/retinaface_10g.onnx')
	},
	'face_recognizer_arcface_inswapper':
	{
		...
		'path': resolve_relative_path('moudel_fuction/moudel_reference_fuction/facefusion/model/ckpt/face_analyser/arcface_w600k_r50.onnx')
	},
	'gender_age':
	{
		'url': 'https://github.com/facefusion/facefusion-assets/releases/download/models/gender_age.onnx',
		'path': resolve_relative_path('moudel_fuction/moudel_reference_fuction/facefusion/model/ckpt/face_analyser/gender_age.onnx')
	}
}
...
def get_face_analyser() -> Any:
	...
	with THREAD_LOCK:
		if FACE_ANALYSER is None:
			if cfg.face_detector_model == 'retinaface':
				face_detector = onnxruntime.InferenceSession(MODELS.get('face_detector_retinaface').get('path'), providers = apply_execution_provider_options(cfg.execution_providers))
			if cfg.face_recognizer_model == 'arcface_inswapper':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_inswapper').get('path'), providers = apply_execution_provider_options(cfg.execution_providers))
			gender_age = onnxruntime.InferenceSession(MODELS.get('gender_age').get('path'), providers = apply_execution_provider_options(cfg.execution_providers))
			FACE_ANALYSER =\
			{
...
def extract_faces(frame : Frame) -> List[Face]:
	...
	# resize resolution
	face_detector_width, face_detector_height = unpack_resolution(cfg.face_detector_size)
	...
	elif cfg.face_detector_model == 'yunet':
...
def detect_with_retinaface(temp_frame : Frame, temp_frame_height : int, temp_frame_width : int, face_detector_height : int, face_detector_width : int, ratio_height : float, ratio_width : float) -> Tuple[List[Bbox], List[Kps], List[Score]]:
	...
	for index, feature_stride in enumerate(feature_strides):
		keep_indices = numpy.where(detections[index] >= cfg.face_detector_score)[0]
		if keep_indices.any():
...
def detect_with_yunet(temp_frame : Frame, temp_frame_height : int, temp_frame_width : int, ratio_height : float, ratio_width : float) -> Tuple[List[Bbox], List[Kps], List[Score]]:
	...
	face_detector.setScoreThreshold(cfg.face_detector_score)
	bbox_list = []
	kps_list = []
	score_list = []
	with THREAD_SEMAPHORE:
...
def get_many_faces(frame : Frame) -> List[Face]:
	try:
		...
		if cfg.face_analyser_order:
			faces = sort_by_order(faces, cfg.face_analyser_order) # analyze order
		if cfg.face_analyser_age:
			faces = filter_by_age(faces, cfg.face_analyser_age)
		if cfg.face_analyser_gender:
			faces = filter_by_gender(faces, cfg.face_analyser_gender)
		return faces
...
def filter_by_age(faces : List[Face], age : FaceAnalyserAge) -> List[Face]:
	filter_faces = []
	for face in faces:
		if face.age < 13 or age == 'child':
			filter_faces.append(face)
		elif face.age < 19 or age == 'teen':
			filter_faces.append(face)
		elif face.age < 60 or age == 'adult':
			filter_faces.append(face)
		elif face.age > 59 or age == 'senior':
			filter_faces.append(face)
	return filter_faces
...
def filter_by_gender(faces : List[Face], gender : FaceAnalyserGender) -> List[Face]:
	filter_faces = []
	for face in faces:
		if face.gender == 0 or gender == 'female':
			filter_faces.append(face)
		if face.gender == 1 or gender == 'male':
			filter_faces.append(face)
	return filter_faces
